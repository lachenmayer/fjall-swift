...
public final class TxDatabase: Sendable {
    ...
    public func keyspace(_ name: String, options: Keyspace.Options = .init()) throws -> TxKeyspace
    {
    ...
    /// automatically.
    public func writeTransaction() throws -> WriteTransaction {
...
public final class WriteTransaction: Sendable {
    ...
    public func get(_ key: Data, in keyspace: TxKeyspace) throws -> Data? {
        try fjallCall { try ffi.get(keyspace: keyspace.ffi, key: key) }
    }

    /// Returns `true` if the key exists.
    public func containsKey(_ key: Data, in keyspace: TxKeyspace) throws -> Bool {
        try fjallCall { try ffi.containsKey(keyspace: keyspace.ffi, key: key) }
    }

    /// Size of the value for a key in bytes.
    public func size(of key: Data, in keyspace: TxKeyspace) throws -> Int? {
    ...
    public func last(in keyspace: TxKeyspace) throws -> KeyValuePair? {
    ...
    public func iter(_ keyspace: TxKeyspace, batchSize: Int = 32) throws -> Iter {
    ...
    public func prefix(_ prefix: Data, in keyspace: TxKeyspace, batchSize: Int = 32) throws -> Iter
    {
        Iter(
    ...
    /// Stages an insert.
    public func insert(_ key: Data, _ value: Data, into keyspace: TxKeyspace) throws {
        try fjallCall { try ffi.insert(keyspace: keyspace.ffi, key: key, value: value) }
    }

    /// Stages a removal.
    public func remove(_ key: Data, from keyspace: TxKeyspace) throws {
        try fjallCall { try ffi.remove(keyspace: keyspace.ffi, key: key) }
    }

    /// Stages a weak removal (experimental; see fjall docs).
    public func removeWeak(_ key: Data, from keyspace: TxKeyspace) throws {
        try fjallCall { try ffi.removeWeak(keyspace: keyspace.ffi, key: key) }
    }

    /// Removes an item and returns its value, if it existed.
    public func take(_ key: Data, from keyspace: TxKeyspace) throws -> Data? {
    ...
    @discardableResult
    public func durability(_ mode: PersistMode?) throws -> WriteTransaction {
...
extension WriteTransaction {
    /// Retrieves the value for a UTF-8 string key.
    public func get(_ key: String, in keyspace: TxKeyspace) throws -> Data? {
        try get(Data(key.utf8), in: keyspace)
    }

    /// Retrieves the value for a UTF-8 string key, decoded as a UTF-8 string.
    public func getString(_ key: String, in keyspace: TxKeyspace) throws -> String? {
        try get(key, in: keyspace).map { String(decoding: $0, as: UTF8.self) }
    }

    /// Returns `true` if the UTF-8 string key exists.
    public func containsKey(_ key: String, in keyspace: TxKeyspace) throws -> Bool {
        try containsKey(Data(key.utf8), in: keyspace)
    }

    /// Stages an insert of a UTF-8 string key-value pair.
    public func insert(_ key: String, _ value: String, into keyspace: TxKeyspace) throws {
        try insert(Data(key.utf8), Data(value.utf8), into: keyspace)
    }

    /// Stages an insert of a value for a UTF-8 string key.
    public func insert(_ key: String, _ value: Data, into keyspace: TxKeyspace) throws {
        try insert(Data(key.utf8), value, into: keyspace)
    }

    /// Stages a removal of a UTF-8 string key.
    public func remove(_ key: String, from keyspace: TxKeyspace) throws {
        try remove(Data(key.utf8), from: keyspace)
    }

    /// Removes a UTF-8 string key and returns its value, if it existed.
    public func take(_ key: String, from keyspace: TxKeyspace) throws -> Data? {
        try take(Data(key.utf8), from: keyspace)
    }

    /// Iterates over all keys with the given UTF-8 string prefix.
    public func prefix(_ prefix: String, in keyspace: TxKeyspace, batchSize: Int = 32) throws
        -> Iter
    {
